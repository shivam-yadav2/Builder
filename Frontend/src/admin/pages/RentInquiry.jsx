...
const RentInquiry = () => {
  ...
  const fetchEnquiries = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/rentFilter/getAll`, {
        headers: authHeader(),
      });
      const payload =
      ...
      const rentEnquiries = processed.filter((item) =>
  ...
  const confirmDelete = async () => {
    ...
    try {
      await toast.promise(
        axios.delete(
          `${API_BASE_URL}/rentFilter/delete/${selectedEnrollment}`,
          { headers: authHeader() }
        ),
        {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-2xl sm:text-3xl font-bold">Rent Inquiries</h1>
