...
const SalesInquiry = () => {
  ...
  const fetchEnquiries = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/sellFilter/getAll`, {
        headers: authHeader(),
      });
      const payload = response?.data?.data ?? response?.data?.message ?? [];
      const sales = (payload || []).filter(
        (item) =>
          item.type?.toLowerCase().includes("home") ||
          item.type?.toLowerCase().includes("land")
      );
      setFilteredEnquiries(sales);
      setError(null);
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await toast.promise(
        axios.delete(
          `${API_BASE_URL}/sellFilter/delete/${selectedEnrollment}`,
          { headers: authHeader() }
        ),
        {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Area (sq ft)",
      cell: ({ row }) =>
        row.original.area ? Number(row.original.area).toLocaleString() : "—",
    },
    {
      accessorKey: "budget",
      header: "Budget (₹)",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const statusInfo =
          STATUS_MAP[row.original.status] || STATUS_MAP.new;
        return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-2xl sm:text-3xl font-bold">Sales Inquiries</h1>
