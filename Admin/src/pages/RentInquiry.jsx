...
const InquiryDashboard = () => {
  ...
  const fetchEnquiries = async () => {
    ...
    try {
      ...
      const config = {
        method: "get",
        url: `${API_BASE_URL}/rentFilter/getAll`,
        headers: {
      ...
      // Filter for rent inquiries (optional, since API is specific)
      const rentEnquiries = processedEnquiries.filter((item) =>
        item.type?.toLowerCase().includes("home")
      );
      setFilteredEnquiries(rentEnquiries);
      setError(null);
    } catch (err) {
      console.error("Error fetching enquiries:", err);
      setError("Unable to load rent inquiries. Please try again later.");
      toast.error("Failed to load rent inquiries");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await toast.promise(
        axios.delete(`${API_BASE_URL}/rentFilter/delete/${selectedEnrollment}`, {
          headers: {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <Card className="shadow-sm">
        <CardHeader className="bg-gray-50 border-b">
          <CardTitle>Rent Inquiries</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {filteredEnquiries.length === 0 ? (
            <div className="p-4 text-center text-gray-500">
              No rent inquiries found
            </div>
          ) : (
            <Table>
              <TableHeader>
